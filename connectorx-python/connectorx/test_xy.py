...
if __name__ == '__main__':
    ...
    # query = "SELECT test_bool, SUM(test_float) FROM test_table WHERE test_int >=2 GROUP BY test_bool"
    # df = read_sql(conn, query)

    # query = "SELECT test_bool, SUM(test_float) FROM test_table GROUP BY test_bool"
    # df = read_sql(conn, query, partition_on="test_int",
    #               partition_num=2, partition_range=(0, 1314))
    # df = read_sql(conn, query, partition_on="test_int",
    #               partition_num=2)

    # query = "CREATE TABLE non_select(id INTEGER NOT NULL)"
    query = "SELECT test_int from test_table; SELECT test_bool from test_table"
    df = read_sql(conn, query)
