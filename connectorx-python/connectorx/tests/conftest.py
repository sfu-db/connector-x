...
@pytest.fixture(scope="session")
def mysql_tls(
    mysql_container: Optional[Any], tmp_path_factory: pytest.TempPathFactory
) -> Optional[tuple[str, str]]:
    """
    Fixture that returns (connection URL, CA certificate path) of a TLS-enabled
    MySQL server, or None when no such server is available.

    Uses MYSQL_URL_TLS / MYSQL_ROOTCERT when set. Otherwise the CA certificate
    auto-generated by the official mysql image on first start is extracted from
    the container started by the `mysql_container` fixture.
    """
    if os.environ.get("MYSQL_URL_TLS"):
        return os.environ["MYSQL_URL_TLS"], os.environ.get("MYSQL_ROOTCERT", "")
    ...
    if result.exit_code != 0:
        pytest.fail(f"failed to read ca.pem from MySQL container: {result.output!r}")
...
@pytest.fixture(scope="module")
def mysql_url_tls(mysql_tls: Optional[tuple[str, str]]) -> str:
    """Fixture that returns the TLS-enabled MySQL connection URL."""
    if mysql_tls is None:
        pytest.skip("No TLS-enabled MySQL database available for tests")
    return mysql_tls[0]


@pytest.fixture(scope="module")
def mysql_rootcert(mysql_tls: Optional[tuple[str, str]]) -> str:
    """Fixture that returns the CA certificate path of the TLS-enabled MySQL server."""
    if mysql_tls is None or not mysql_tls[1]:
        pytest.skip("No TLS-enabled MySQL database available for tests")
    return mysql_tls[1]
