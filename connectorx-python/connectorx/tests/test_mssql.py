...
def test_read_sql_tls(mssql_url_tls: str) -> None:
    query = "SELECT * FROM test_table"
    df = read_sql(
        f"{mssql_url_tls}?sslmode=require",
        query,
        partition_on="test_int",
        partition_range=(0, 2000),
        partition_num=3,
...
def test_read_sql_tls_with_cert(mssql_url_tls: str, postgres_rootcert: str) -> None:
    query = "SELECT * FROM test_table"
    df = read_sql(
        f"{mssql_url_tls}?sslmode=require&sslrootcert={postgres_rootcert}",
        query,
        partition_on="test_int",
        partition_range=(0, 2000),
        partition_num=3,
...
def test_read_sql_tls_disable(mssql_url_tls: str) -> None:
    query = "SELECT * FROM test_table"
    df = read_sql(
        f"{mssql_url_tls}?sslmode=disable",
        query,
        partition_on="test_int",
        partition_range=(0, 2000),
        partition_num=3,
...
@pytest.mark.xfail
def test_read_sql_tls_fail(mssql_url_tls: str) -> None:
    query = "SELECT * FROM test_table"
    df = read_sql(
        f"{mssql_url_tls}?sslmode=require&sslrootcert=fake.cert",
        query,
        partition_on="test_int",
        partition_range=(0, 2000),
        partition_num=3,
