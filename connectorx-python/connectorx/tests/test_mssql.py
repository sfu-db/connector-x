...
def test_memory(mssql_url: str) -> None:
    ...
    table = None
    query = "SELECT top 1 * FROM test_table"
    last_mem = psutil.Process().memory_info()
    init_mem = last_mem
    for x in range(0, 100):
        print(f"run #{x}")
        temp_table: pa.Table = read_sql(mssql_url, query, return_type="arrow")
        print("tmp_table ref count:", sys.getrefcount(temp_table))

        # when enabled, the the virtual memory explosion doesn't occur
        # temp_table = copy.deepcopy(temp_table)
        now_mem = psutil.Process().memory_info()
        print(
            f"before concat now mem: {now_mem}, rss + {now_mem.rss-last_mem.rss}, vms + {now_mem.vms - last_mem.vms}"
        )
        last_mem = now_mem
        ...
        now_mem = psutil.Process().memory_info()
        print(
            f"after concat now mem: {now_mem}, rss + {now_mem.rss-last_mem.rss}, vms + {now_mem.vms - last_mem.vms}"
        )
        last_mem = now_mem

    now_mem = psutil.Process().memory_info()
    print(
        f"final: {now_mem}, rss + {now_mem.rss-init_mem.rss}, vms + {now_mem.vms - init_mem.vms}"
    )


def test_arrow2(mssql_url: str) -> None:
    import pyarrow as pa
    import psutil
    import sys
    import copy
    ...
    for x in range(0, 1000):
        print(f"run #{x}")
        temp_table: pa.Table = read_sql(mssql_url, query, return_type="arrow2")
...
def test_good(mssql_url: str) -> None:
    import pyarrow as pa
    import psutil
    import copy
    import sys
    ...
    for x in range(0, 10):
        ...
        # when enabled, the the virtual memory explosion doesn't occur
        temp_table = copy.deepcopy(temp_table)
        print("tmp_table ref count after deep copy:", sys.getrefcount(temp_table))
