...
use pyo3::{
    ...
    Py, PyObject, PyResult, Python, ToPyObject,
};
use rayon::prelude::*;
...
fn single_thread_batch_alloc(py: Python, strings: &[&str]) -> Result<(), Error> {
    let mut view = create_numpy_object_array(py, strings.len()).unwrap();
    for (i, s) in strings.iter().enumerate() {
        view[i] = pystring::new(py, s.len()).to_object(py);
    }

    let mut views = vec![];
    while view.len() != 0 {
        let size = view.len().min(50000);
    ...
    strings
        ...
        .for_each(|(src, dst)| unsafe {
            ...
            for i in 0..src.len() {
                pystring::write(dst[i].as_ref(py).downcast::<PyString>().unwrap(), &src[i])
            }
        });
