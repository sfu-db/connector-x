    ...
    ($proto:ty, $tls:ty) => {
        impl_transport!(
            ...
            mappings = {
                ...
                { NumericArray[Vec<Option<Decimal>>] => DecimalArray[Vec<Option<Decimal>>]     | conversion option }
            }
        );
    }
}
...
impl<P, C> TypeConversion<Vec<Option<Decimal>>, Vec<Option<Decimal>>> for PostgresArrowTransport<P, C> {
    fn convert(val: Vec<Option<Decimal>>) -> Vec<Option<Decimal>> {
        val
    }
}
