use connectorx::{
    destinations::memory::MemoryDestination, prelude::*, sources::mssql::MsSQLSource, sql::CXQuery,
    transports::MsSQLMemoryTransport,
};
use ndarray::array;
use std::env;
use std::sync::Arc;
...
fn load_and_parse() {
    ...
    let mut source = MsSQLSource::new(rt, &dburl, 1).unwrap();
...
fn test_mssql() {
    ...
    let dispatcher =
        Dispatcher::<_, _, MsSQLMemoryTransport>::new(builder, &mut destination, &queries);
